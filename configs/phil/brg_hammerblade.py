...
n_cols = int(options.num_col)

# this extra row of routers is for the l2s
#n_rows  = n_cols + 1
n_rows_ori = int(options.num_row) 
print("number of rows orignal: ",n_rows_ori)
n_rows = n_rows_ori + 1
n_l2s   = n_cols
...
print("net width is: ",options.net_width)  
# network classes
#assert(options.network == "garnet2.0")
options.network = "garnet2.0"
virt_channels = 4
options.vcs_per_vnet = virt_channels
#options.router_latency = 5
#options.link_latency = 200
#options.routing_algorithm = 1
#options.num_rows = 14
#options.num_cols = 8
#options.ni_flit_size = 32
#options.link_width_bits = 2048
#options.mesh_rows = num_rows
...
#network.vcs-per-vnet = options.vcs-per-vnet
# Scratchpads
...
#system.mem_ctrl.latency = '60ns' 
#system.mem_ctrl.bandwidth = '2048GB/s'
